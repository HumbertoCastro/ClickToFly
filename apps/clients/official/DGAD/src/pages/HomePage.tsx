...
const pillarReferenceById = new Map(
  pillars.map((pillar) => [
    pillar.id,
    {
      title: pillar.referenceTitle,
      url: pillar.referenceUrl,
    },
  ]),
);
    ...
    <div ref={rootRef} className="home-landing">
      ...
      <section className="lf-section lf-section--light benefits-section" aria-labelledby="benefits-title">
        ...
        <div className="benefit-bento">
          {benefits.map(({ pillarId, title, text, Icon, image }, index) => {
            const reference = pillarReferenceById.get(pillarId);

            return (
              <a
                key={title}
                className={`benefit-card benefit-card--${index}`}
                href={reference?.url ?? offer.checkoutUrl}
                target="_blank"
                rel="noreferrer"
                style={{ "--benefit-bg": `url(${image})` } as CSSProperties}
                aria-label={`Abrir livro de referência: ${reference?.title ?? title}`}
              >
                <div className="benefit-card__content">
                  <Icon aria-hidden="true" />
                  <h3>{title}</h3>
                  <p>{text}</p>
                  <span className="benefit-card__link">
