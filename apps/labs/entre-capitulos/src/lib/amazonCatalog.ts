...
export function getAmazonCatalogConfig(
  ...
  if (mode !== "disabled" && !endpoint) {
    throw new AmazonCatalogError(
      "VITE_AMAZON_CATALOG_ENDPOINT ou VITE_SUPABASE_URL é obrigatório quando a Livraria Amazon está habilitada.",
...
export function createAmazonFallbackBook(
  asin: string,
): AmazonLibraryBookDraft {
  ...
  return {
    source: "amazon",
    sourceId: normalizedAsin,
    title: `Livro Amazon ${normalizedAsin}`,
    subtitle: "",
